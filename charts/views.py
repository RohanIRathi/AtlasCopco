...
def visitor_chart2(request):
    ...
    result = reversed(
        Visitor.objects.annotate(month=TruncMonth('in_time')).values('month').annotate(c=Count('id')).values('month',
                                                                                                             'c'))
...
def visitor_chart3(request):
    ...
    result = reversed(
        Visitor.objects.annotate(year=TruncYear('in_time')).values('year').annotate(c=Count('id')).values('year', 'c'))
